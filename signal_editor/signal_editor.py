...
class SignalEditor(QtWidgets.QApplication):
    def __init__(self, args: list[str]) -> None:
        ...
        self.plot_controller = PlotController(self, self.mw)
    ...
    def connect_qt_signals(self) -> None:
        ...
        self.mw.metadata_dialog.sig_property_has_changed.connect(self.update_metadata)
        ...
        self.mw.action_toggle_auto_scaling.toggled.connect(self.plot_controller.toggle_auto_scaling)
        self.mw.action_show_filter_inputs.triggered.connect(self.show_processing_inputs_dock)
        self.mw.action_show_section_overview.toggled.connect(self.plot_controller.toggle_regions)
    ...
    def _on_sig_new_data(self) -> None:
        self.mw.dock_section_list.list_view.setModel(self.data_controller.sections)
        self.mw.dock_section_list.list_view.setCurrentIndex(self.data_controller.base_section_index)
        self.update_sampling_rate()
    ...
    def _connect_data_controller_signals(self) -> None:
        ...
        self.mw.dock_section_list.list_view.pressed.connect(self.data_controller.set_active_section)
    ...
    def show_metadata_dialog(self, required_fields: list[str]) -> None:
        ...
        self.mw.metadata_dialog.combo_box_signal_column.setModel(metadata.columns)
        self.mw.metadata_dialog.combo_box_info_column.setModel(metadata.columns)
        if "sampling_rate" not in required_fields:
            sampling_rate = metadata.sampling_rate
            self.mw.metadata_dialog.spin_box_sampling_rate.setValue(sampling_rate)
        ...
        if "signal_column" not in required_fields:
            ...
            self.mw.metadata_dialog.combo_box_signal_column.setCurrentText(signal_col)
            self.mw.combo_box_signal_column_import_page.setCurrentText(signal_col)

        self.mw.metadata_dialog.combo_box_info_column.setCurrentText(info_col)
        self.mw.combo_box_info_column_import_page.setCurrentText(info_col)

        self.mw.metadata_dialog.line_edit_file_name.setText(file_name)
        self.mw.metadata_dialog.line_edit_file_type.setText(file_type)

        self.mw.metadata_dialog.open()
    ...
    def _set_column_models(self) -> None:
        self.mw.combo_box_info_column_import_page.setModel(self.data_controller.metadata.columns)
        self.mw.combo_box_signal_column_import_page.setModel(self.data_controller.metadata.columns)
        self.mw.metadata_dialog.combo_box_signal_column.setModel(
            self.data_controller.metadata.columns
        )
        self.mw.metadata_dialog.combo_box_info_column.setModel(
            self.data_controller.metadata.columns
        )
    ...
    def _clear_column_models(self) -> None:
        ...
        with QtCore.QSignalBlocker(self.mw.metadata_dialog.combo_box_info_column):
            self.mw.metadata_dialog.combo_box_signal_column.clear()
        with QtCore.QSignalBlocker(self.mw.metadata_dialog.combo_box_signal_column):
            self.mw.metadata_dialog.combo_box_info_column.clear()
    ...
    def open_file(self) -> None:
        ...
        self.mw.table_view_import_data.setModel(self.data_controller.data_model)
