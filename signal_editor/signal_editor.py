...
class SignalEditor(QtWidgets.QApplication):
    def __init__(self, args: list[str]) -> None:
        ...
        self.mw = MainWindow()
        self.dctrl = DataController(self)
        self.pctrl = PlotController(self, self.mw)
    ...
    def connect_qt_signals(self) -> None:
        ...
        self.mw.action_toggle_auto_scaling.toggled.connect(self.pctrl.toggle_auto_scaling)
        self.mw.action_show_processing_inputs.toggled.connect(
            self.mw.dock_processing_inputs.setVisible
        )
        self.mw.action_show_peak_detection_inputs.toggled.connect(self.mw.dock_peaks.setVisible)
        self.mw.action_show_section_overview.toggled.connect(self.pctrl.toggle_regions)

        self.mw.dock_section_list.list_view.sig_delete_current_item.connect(self.delete_section)
        self.mw.dock_processing_inputs.sig_filter_requested.connect(self.filter_active_signal)
        self.mw.dock_processing_inputs.sig_pipeline_requested.connect(self.run_preprocess_pipeline)
        self.mw.dock_processing_inputs.sig_standardization_requested.connect(
            self.standardize_active_signal
        )
        self.mw.dock_processing_inputs.sig_data_reset_requested.connect(
    ...
    def filter_active_signal(self, filter_params: _t.SignalFilterParameters) -> None:
        self.dctrl.active_section.filter_signal(pipeline=PreprocessPipeline.Custom, **filter_params)
        self.refresh_plot_data()

    @QtCore.Slot()
    def restore_original_signal(self) -> None:
        self.dctrl.active_section.reset_signal()
        self.refresh_plot_data()
    ...
    def run_preprocess_pipeline(self, pipeline: PreprocessPipeline) -> None:
        if pipeline not in {PreprocessPipeline.PPGElgendi, PreprocessPipeline.ECGNeuroKit2}:
            return
        self.dctrl.active_section.filter_signal(pipeline)
        self.refresh_plot_data()
    ...
    def standardize_active_signal(
        ...
        self.dctrl.active_section.scale_signal(method=method, robust=robust, window_size=window_size)
        self.refresh_plot_data()

    def refresh_plot_data(self) -> None:
        self.pctrl.set_signal_data(self.dctrl.active_section.processed_signal.to_numpy())
    ...
    def run_peak_detection(
        ...
        self.dctrl.active_section.detect_peaks(method, params)
        self.pctrl.set_peak_data(*self.dctrl.active_section.get_peak_xy())
        self.pctrl.set_rate_data(self.dctrl.active_section.rate_instantaneous_interpolated)

    @QtCore.Slot()
    def _on_sig_new_data(self) -> None:
        self.mw.dock_section_list.list_view.setModel(self.dctrl.sections)
        self.mw.dock_section_list.list_view.setCurrentIndex(self.dctrl.base_section_index)
        self.update_sampling_rate()

    def _connect_data_controller_signals(self) -> None:
        self.dctrl.sig_user_input_required.connect(self.show_metadata_dialog)
        self.dctrl.sig_new_metadata.connect(self.update_metadata_widgets)
        self.dctrl.sig_new_data.connect(self._on_sig_new_data)
        self.mw.dock_section_list.list_view.pressed.connect(self.dctrl.set_active_section)
        self.dctrl.sig_active_section_changed.connect(self._on_active_section_changed)
    ...
    def _disconnect_data_controller_signals(self) -> None:
        ...
        safe_disconnect(
            self.mw.dock_section_list.list_view,
            self.mw.dock_section_list.list_view.pressed,
            self.dctrl.set_active_section,
        )
    ...
    def maybe_new_section(self, checked: bool) -> None:
        ...
        if not checked:
            self.pctrl.hide_region_selector()
            return
        bounds = (0, self.dctrl.base_df.height - 1)
        self.pctrl.show_region_selector(bounds)

    @QtCore.Slot()
    def create_new_section(self) -> None:
        if self.pctrl.region_selector is None:
            return
        if not self.pctrl.region_selector.isVisible():
            return
        start, stop = self.pctrl.region_selector.getRegion()
        self.dctrl.create_section(start, stop)
        self.pctrl.hide_region_selector()
    ...
    def delete_section(self, index: QtCore.QModelIndex) -> None:
        self.dctrl.delete_section(index)
        logger.success(f"Deleted section {index.row():03}")

    @QtCore.Slot(bool)
    def _on_active_section_changed(self, has_peaks: bool) -> None:
        section = self.dctrl.active_section
        is_base_section = section is self.dctrl.get_base_section()
        self.mw.action_create_new_section.setEnabled(is_base_section)
        self.pctrl.set_signal_data(section.processed_signal.to_numpy())
        if has_peaks:
            self.pctrl.set_peak_data(*section.get_peak_xy())
            self.pctrl.set_rate_data(section.rate_instantaneous_interpolated)

    @QtCore.Slot()
    def show_processing_inputs_dock(self) -> None:
        self.mw.dock_processing_inputs.show()

    @QtCore.Slot()
    def show_peak_detection_dock(self) -> None:
        self.mw.dock_peaks.show()
    ...
    def update_metadata(self, metadata_dict: _t.MetadataUpdateDict) -> None:
        ...
        self.dctrl.update_metadata(sampling_rate, signal_col, info_col)
    ...
    def update_sampling_rate(self) -> None:
        ...
        self.dctrl.update_metadata(sampling_rate=sampling_rate)
        self.pctrl.update_time_axis_scale(sampling_rate)
        self.mw.dock_processing_inputs.update_frequency_sliders(sampling_rate)
        logger.info(f"Sampling rate set to {sampling_rate} Hz.")

    @QtCore.Slot(str)
    def update_signal_column(self, signal_column: str) -> None:
        self.dctrl.update_metadata(signal_col=signal_column)
        logger.info(f"Signal column set to '{signal_column}'.")

    @QtCore.Slot(str)
    def update_info_column(self, info_column: str) -> None:
        self.dctrl.update_metadata(info_col=info_column)
        logger.info(f"Info column set to '{info_column}'.")

    def _set_column_models(self) -> None:
        self.mw.combo_box_info_column_import_page.setModel(self.dctrl.metadata.columns)
        self.mw.combo_box_signal_column_import_page.setModel(self.dctrl.metadata.columns)
        self.mw.dialog_meta.combo_box_signal_column.setModel(self.dctrl.metadata.columns)
        self.mw.dialog_meta.combo_box_info_column.setModel(self.dctrl.metadata.columns)

        with contextlib.suppress(Exception):
            self.mw.combo_box_info_column_import_page.setCurrentText(self.dctrl.metadata.info_column)
            self.mw.combo_box_signal_column_import_page.setCurrentText(
                self.dctrl.metadata.signal_column
            )
    ...
    def open_file(self) -> None:
        ...
        self.dctrl.open_file(file_path)
        self.mw.table_view_import_data.setModel(self.dctrl.data_model)
        self._set_column_models()

    @QtCore.Slot()
    def read_data(self) -> None:
        self.dctrl.load_data()
        for col in range(self.dctrl.base_df.width):
    ...
    def close_file(self) -> None:
        ...
        self.mw.dock_section_list.list_view.setModel(None)

        with contextlib.suppress(Exception):
            self._disconnect_data_controller_signals()
            self.dctrl.setParent(None)
    ...
    def _update_setting(self, name: str, value: QtGui.QColor | str | int | float | None) -> None:
        ...
        match name:
            case "background_color":
                self.pctrl.set_background_color(value)
            case "foreground_color":
                self.pctrl.set_foreground_color(value)
            case "point_color":
                if self.pctrl.peak_scatter is not None:
                    self.pctrl.peak_scatter.setBrush(value)
            case "signal_line_color":
                if self.pctrl.signal_curve is not None:
                    self.pctrl.signal_curve.setPen(value)
            case "rate_line_color":
                if self.pctrl.rate_curve is not None:
                    self.pctrl.rate_curve.setPen(value)
            case "section_marker_color":
                for r in self.pctrl.regions:
                    r.setBrush(color=value)
            case "float_visual_precision":
                if isinstance(value, int):
                    self.dctrl.data_model.set_float_precision(value)
            case _:
                pass

    @QtCore.Slot()
    def apply_settings(self) -> None:
        self.pctrl.apply_settings()
