...
class CollapsibleFrame(QtWidgets.QFrame):
    ...
    toggled: t.ClassVar[QtCore.Signal] = QtCore.Signal(bool)

    def __init__(
        self,
        title: str = "",
        parent: QtWidgets.QWidget | None = None,
        expandedIcon: QtGui.QIcon | str | None = None,
        collapsedIcon: QtGui.QIcon | str | None = None,
    ):
        ...
        self._toggle_btn = qfw.TogglePushButton(title)
        if expandedIcon is None:
            expandedIcon = Icons.ChevronDown.icon()

        if collapsedIcon is None:
            collapsedIcon = Icons.ChevronRight.icon()
        ...
        _content.layout().setContentsMargins(QtCore.QMargins(5, 0, 0, 0))
        self.setContent(_content)

    def toggleButton(self) -> qfw.TogglePushButton:
    ...
    def eventFilter(self, watched: QtCore.QObject, event: QtCore.QEvent) -> bool:
        ...
        if event.type() == QtCore.QEvent.Type.Resize and self.isExpanded() and not self._is_animating:
            self._expand_collapse(QtCore.QPropertyAnimation.Direction.Forward, animate=False, emit=False)
        return False
